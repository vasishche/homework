...
# 4. download hadoop
curl https://archive.apache.org/dist/hadoop/common/hadoop-3.1.2/hadoop-3.1.2.tar.gz -o 'hadoop.tar.gz'
...
sed -i -r "s,^(export JAVA_HOME=).*,\1\"$(dirname $(dirname $(readlink -f $(which java))))\",g;s,^(export HADOOP_HOME=).*,\1\"/usr/local/hadoop/current\",g;s/^(export HADOOP_HEAPSIZE_MAX=).*/\1\"512M\"/g" conf/hadoop-env.sh
sed -i -r 's/^(.*)%HDFS_NAMENODE_HOSTNAME%(.*)/\1vm1-headnode\2/g' conf/core-site.xml 
sed -i -r 's#^(.*)%NAMENODE_DIRS%(.*)#\1/opt/mount1/namenode-dir,/opt/mount2/namenode-dir\2#g;s#^(.*)%DATANODE_DIRS%(.*)#\1/opt/mount1/datenode-dir,/opt/mount2/datenode-dir\2#g' conf/hdfs-site.xml 
sed -i -r 's/^(.*)%YARN_RESOURCE_MANAGER_HOSTNAME%(.*)/\1vm1-headnode\2/g;s#^(.*)%NODE_MANAGER_LOCAL_DIR%(.*)#\1/opt/mount1/nodemanager-local-dir,/opt/mount2/nodemanager-local-dir\2#g;s#^(.*)%NODE_MANAGER_LOG_DIR%(.*)#\1/opt/mount1/nodemanager-log-dir,/opt/mount2/nodemanager-log-dir\2#g' conf/yarn-site.xml
sudo cp conf/{hadoop-env.sh,{core,hdfs,yarn}-site.xml} /usr/local/hadoop/current/etc/hadoop/

# 24. create hadoop.sh in /etc/profile.d/
